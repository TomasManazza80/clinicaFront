...
const deleteCirugia = async (id) => {
  try {
    ...
    if (response.ok) {
      ...
      setCirugias(cirugias.filter(cirugia => cirugia.id !== id)); // Actualiza el estado cirugias
    } else {
...
const eliminarFoto = async (id) => {
  try {
    ...
    setFotos(fotos.filter(foto => foto.galleryId !== id)); // Actualiza el estado fotos
...
const handleDeleteService = async (id) => {
  try {
    ...
    setServices(services.filter(service => service.id !== id)); // Actualiza el estado services
  } catch (error) {
...
const deleteDoctor = async (id) => {
  try {
    ...
    setDoctores(doctores.filter(doctor => doctor.doctorId !== id)); // Actualiza el estado doctores
  } catch (error) {
      ...
      <form onSubmit={handleSubmit}>
        ...
        <div style={{ fontFamily: "Arial, sans-serif", padding: "20px", maxWidth: "800px", margin: "auto", backgroundColor: "#f9f9f9", borderRadius: "8px", boxShadow: "0 0 10px rgba(0, 0, 0, 0.1)" }}>
          ...
          <div style={{ marginBottom: "20px" }}>
            ...
            {doctores.map((doctor) => (
              <div key={doctor.doctorId} style={{ marginBottom: "15px", padding: "10px", backgroundColor: "#fff", border: "1px solid #ccc", borderRadius: "4px" }}>
